...
def remove_punc_stopword(text):  #对文本数据进行清洗
    """
...
def main(args):
    ...
    train_comments, test_comments, train_labels, test_labels  = train_test_split(list(Review), list(Rating), test_size = 0.25, random_state = 168)
    #------------------------------------------------------------------------
    ...
    plt.plot(np.linspace(0,20,20), Train_acc, "ro-", label="train acc") 
    plt.plot(np.linspace(0,20,20), Test_acc, "bs-", label="test acc")
    plt.legend()
    plt.xlabel("Epoch")
    plt.ylabel("Accuracy")
    plt.title("Bert")
...
if __name__ == '__main__':  ### 判断当前的运行环境是否是直接运行的主程序
    ...
    parser.add_argument('--batch_size', type=int, default=5)      #训练批次
